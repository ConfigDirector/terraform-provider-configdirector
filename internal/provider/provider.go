	...
	resp.Schema = schema.Schema{
		Attributes: map[string]schema.Attribute{
			"base_url": schema.StringAttribute{
				Optional:    true,
				Description: "Base URL of the ConfigDirector API, e.g. https://api.configdirector.com. Defaults to " + defaultBaseUrl + " and it is overridable by the CONFIGDIRECTOR_BASE_URL environment variable. Generally, it is only needed if connecting via a ConfigDirector proxy or relay.",
			},
